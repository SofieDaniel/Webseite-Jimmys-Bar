...
def test_cms_locations_get():
    ...
    try:
        ...
        print(f"📊 Locations content:")
        if "page_title" in data:
            print(f"  Page title: {data['page_title']}")
        if "page_description" in data:
            print(f"  Page description: {data['page_description']}")
...
def test_cms_about_get():
    ...
    try:
        ...
        # Print about details
        print(f"📊 About content:")
        if "hero_title" in data:
            print(f"  Hero title: {data['hero_title']}")
        if "story_content" in data:
            print(f"  Story content: {data['story_content'][:50]}...")
        if "team_members" in data:
            print(f"  Number of team members: {len(data['team_members'])}")
        if "values" in data:
            print(f"  Number of values: {len(data['values'])}")
...
def run_menu_validation_tests():
    ...
    if auth_success:
        ...
        locations_success = test_cms_locations_get()
        results["cms_locations"] = locations_success
        
        # Test CMS About endpoint
        about_success = test_cms_about_get()
